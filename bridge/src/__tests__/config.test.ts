...
describe("config endpoints", () => {
  ...
  describe("/rules endpoints (alwaysAllowRules)", () => {
    async function getRules() {
      const res = await fetch(`${baseUrl}/rules`, { headers: { "x-decky-token": token } });
      return { status: res.status, data: await res.json() };
    }
    async function postRule(pattern: string) {
    ...
    async function deleteRule(id: string) {
      const res = await fetch(`${baseUrl}/rules/${id}`, {
        method: "DELETE",
        headers: { "x-decky-token": token },
      });
      return { status: res.status, data: await res.json() };
    ...
    it("GET /rules returns empty rules list initially", async () => {
      const { status, data } = await getRules();
      expect(status).toBe(200);
      expect(data.rules).toEqual([]);
    });

    it("POST /rules adds a rule and returns it", async () => {
      const { status, data } = await postRule("Bash");
      expect(status).toBe(200);
      expect(data.ok).toBe(true);
      expect(data.rules).toHaveLength(1);
      expect(data.rules[0].pattern).toBe("Bash");
      expect(typeof data.rules[0].id).toBe("string");
      expect(typeof data.rules[0].createdAt).toBe("number");
    });

    it("POST /rules with empty pattern returns 400", async () => {
      const { status } = await postRule("   ");
      expect(status).toBe(400);
    });

    it("POST /rules with missing pattern returns 400", async () => {
      const res = await fetch(`${baseUrl}/rules`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "x-decky-token": token },
        body: JSON.stringify({}),
    ...
    it("DELETE /rules/:id removes the rule", async () => {
      const { data: added } = await postRule("Read");
      const id = added.rules.find((r: { pattern: string }) => r.pattern === "Read")?.id;
      expect(id).toBeDefined();
      const { status, data } = await deleteRule(id);
      expect(status).toBe(200);
      expect(data.ok).toBe(true);
      expect(data.rules.find((r: { id: string }) => r.id === id)).toBeUndefined();
    });

    it("DELETE /rules/:id for non-existent id is a no-op", async () => {
      const { status, data } = await deleteRule("nonexistent-id");
      expect(status).toBe(200);
      expect(data.ok).toBe(true);
    });

    it("GET /rules after adding reflects current list", async () => {
